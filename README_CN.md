...
| 功能 | 说明 |
|---|---|
| **Antigravity 2.0 兼容** | 同时支持 Antigravity 1.x 和 2.0（standalone hub 模式，`source` 字段注入） |
| **多实例 IDE 选择器** | 侧边栏可切换多个正在运行的 Antigravity 实例 |
| **手动刷新按钮** | 侧边栏一键刷新对话列表，无需等待自动轮询 |
| **移动端输入框修复** | 修复软键盘弹出或全面屏手势栏导致输入框被截断的问题（动态视口高度 + 安全区适配） |
...
- **2026-06-01** — 合并上游 [porta](https://github.com/L1M80/porta) 最新更改；新增 Antigravity 2.0 兼容、多实例 IDE 选择器、手动刷新按钮；修复全面屏设备输入框截断问题
...
- **2.0 版本记录混放** —— Antigravity 1.x 和 2.0 的对话历史存储在同一个全局目录中。当两个版本同时运行时，对话列表可能混合显示，指定 2.0 实例时可能出现加载延迟。切换到"auto"模式可解决大部分问题
